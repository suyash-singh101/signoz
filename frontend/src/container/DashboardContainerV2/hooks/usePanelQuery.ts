...
export interface UsePanelQueryResult {
	/**
	 * V1 in-memory Query produced by running the perses → V1 adapter over the
	 * panel's queries. Always defined — fromPerses inflates to the empty
	 * composite shape `{queryData: [], queryFormulas: [], queryTraceOperator: []}`
	 * when there are no queries, so consumers never need a null check.
	 */
	query: Query;
...
/**
 * One-stop hook for "give me this V2 panel's V1 Query + its query-range data."
 *
 ...
 *      the V1 in-memory Query shape `useGetQueryRange` consumes.
