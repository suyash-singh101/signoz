...
describe('usePanelQuery', () => {
	it('runs fromPerses on the panel queries and exposes the V1 Query', () => {
		const { result } = renderHook(() =>
			usePanelQuery({ panel: builderPanel(), panelId: 'p1' }),
		);
		expect(result.current.query.queryType).toBe(EQueryType.QUERY_BUILDER);
		expect(result.current.query.builder.queryData).toHaveLength(1);
		expect(result.current.query.builder.queryData[0].queryName).toBe('A');
	});
	...
	it('returns the inflated empty composite when panel is undefined (no crash)', () => {
		const { result } = renderHook(() =>
			usePanelQuery({ panel: undefined, panelId: 'p-none' }),
		);
		expect(result.current.query.queryType).toBe(EQueryType.QUERY_BUILDER);
		expect(result.current.query.builder.queryData).toStrictEqual([]);
	});
});
